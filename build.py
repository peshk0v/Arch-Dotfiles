...
os.system(f"cp {pathlib.Path(__file__).parent.resolve()}/.config/* {pathlib.Path.home()}/.config/")
...
if pathlib.Path(f"{pathlib.Path.home()}/Pictures").is_dir():
    os.system(f"cp -r {pathlib.Path(__file__).parent.reslove()}/data/Wallpapers {pathlib.Path.home()}/Pictures/")
else:
